...
const getProblemsForDate = async (date, viewerStudentId, groupId = null) => {
  const challengeDate = toDateString(date);
  const dayStart = startOfDayUTC(challengeDate);
  const dayEnd = endOfDayUTC(challengeDate);
  const dateFilter = { $or: [{ challengeDate }, { challengeDate: { $exists: false }, date: { $gte: dayStart, $lte: dayEnd } }] };
  const query = groupId
    ? { ...dateFilter, groupId: mongoose.Types.ObjectId.isValid(groupId) ? new mongoose.Types.ObjectId(groupId) : null }
    : dateFilter;
...
const createProblem = async ({ studentId, leetcodeUrl, date, groupId = null }) => {
  ...
  const existing = await Problem.findOne({ studentId, challengeDate, groupId: groupId || null });
  if (existing) {
    const error = new Error('Student already submitted a problem for this date in this group');
  ...
  const problem = await Problem.create({
    ...
    date: problemDate,
    leetcodeUrl,
    title: titleFromUrl(leetcodeUrl),
    deadline,
  });
...
const completeProblem = async (problemId, studentId) => {
  ...
  const { pointsEarned, isOnTime } = await calculatePoints(
    completedAt,
    problem.deadline,
    problem.challengeDate || toDateString(problem.date),
    problemId,
  ...
  const completion = await Completion.findOneAndUpdate(
    { problemId, studentId },
    {
      $set: { status: 'completed', completedAt, isOnTime, pointsEarned, groupId: problem.groupId || null },
...
const saveProgressNote = async (problemId, studentId, note) => {
  ...
  const progress = await Completion.findOneAndUpdate(
    { problemId, studentId },
    {
      $set: { note, groupId: problem.groupId || null },
      $setOnInsert: {
        challengeDate: problem.challengeDate || toDateString(problem.date),
        status: 'pending', pointsEarned: 0, groupId: problem.groupId || null,
      },
    },
    { new: true, upsert: true, setDefaultsOnInsert: true }
  );
